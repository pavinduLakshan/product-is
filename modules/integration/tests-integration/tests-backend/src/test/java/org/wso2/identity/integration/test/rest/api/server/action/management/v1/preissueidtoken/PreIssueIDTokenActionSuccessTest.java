...
/**
 * Tests for happy paths of the Pre Issue ID Token Action Management REST API.
 */
public class PreIssueIDTokenActionSuccessTest extends PreIssueIDTokenTestBase {

    private static ActionModel action;
    private static String testActionId;

    @Factory(dataProvider = "restAPIUserConfigProvider")
    public PreIssueIDTokenActionSuccessTest(TestUserMode userMode) throws Exception {
        ...
        responseOfGet.then()
                ...
                .body("find { it.id == '" + testActionId + "' }.links.find { it.rel == 'self' }.href",
                        equalTo(buildBaseURL() + ACTION_MANAGEMENT_API_BASE_PATH +
                                PRE_ISSUE_ID_TOKEN_PATH + "/" + testActionId))
    ...
    public void testGetActionTypes() {
        ...
        responseOfGet.then()
                ...
                .assertThat()
                .statusCode(HttpStatus.SC_OK)
                .body( "find { it.type == '" + PRE_ISSUE_ID_TOKEN_ACTION_TYPE + "' }.displayName",
                        notNullValue())
                .body( "find { it.type == '" + PRE_ISSUE_ID_TOKEN_ACTION_TYPE + "' }.description",
                        notNullValue())
                .body( "find { it.type == '" + PRE_ISSUE_ID_TOKEN_ACTION_TYPE + "' }.count",
                        equalTo(1))
                .body( "find { it.type == '" + PRE_ISSUE_ID_TOKEN_ACTION_TYPE + "' }.self", notNullValue());
    }
    ...
    public void testUpdateActionUpdatingExistingRule() {
        ...
        responseOfPatch.then()
                ...
                .body("rule.rules[1].expressions[0].value", equalTo("refresh_token"));
    }
    ...
    public void testDeleteAction() {
        ...
        getResponseOfDelete(ACTION_MANAGEMENT_API_BASE_PATH + PRE_ISSUE_ID_TOKEN_PATH + "/" + testActionId)
                .then()
                .log().ifValidationFails()
                .assertThat()
